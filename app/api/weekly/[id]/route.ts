...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const goal = await getGoalForUser(params.id, me.id);
  ...
  const updated = await db.weeklyGoal.update({ where: { id: params.id }, data });
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await db.weeklyGoal.delete({ where: { id: params.id } });
