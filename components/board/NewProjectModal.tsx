...
/**
 * NewProjectModal — extracted into its own file so Turbopack always compiles
 * it fresh (no stale build-cache entry can exist for a brand-new file).
 */
...
export function NewProjectModal({ onClose }: Props) {
  ...
  return (
    /* np-overlay / np-card are in globals.css and handle
       desktop (centered) vs mobile (bottom-sheet) via @media */
    <div className="np-overlay" onClick={onClose}>
      ...
      >
        ...
        {/* Tab switcher — always rendered on all screen sizes */}
        <div
